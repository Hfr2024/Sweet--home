...
function render(){
  ...
  if(logo&&D.site.logo)logo.src=D.site.logo;

  const hero=document.querySelector(".hero");
  if(hero&&D.site.hero)hero.style.backgroundImage=`url("${D.site.hero}")`;
  ...
  if(new URLSearchParams(location.search).get("admin")==="1")$("adminOpen").classList.remove("hidden");
  else $("adminOpen").classList.add("hidden");
}
...
/* رفع الصور من معرض الهاتف مع ضغطها تلقائيًا */
function compressImage(file,callback){
  ...
  if(file.size>5*1024*1024){
    alert("الصورة كبيرة جدًا. اختار صورة أقل من 5MB.");
    return;
  }
  ...
  reader.onload=()=>{
    img.onload=()=>{
      ...
      callback(c.toDataURL("image/jpeg",0.78));
    };
    img.onerror=()=>alert("تعذر قراءة الصورة. اختار صورة أخرى.");
    img.src=reader.result;
  };
  reader.onerror=()=>alert("تعذر قراءة الصورة.");
  reader.readAsDataURL(file);
}

function pickImage(input,index){
  const file=input.files&&input.files[0];
  if(!file)return;
  compressImage(file,data=>{
    D.products[index].image=data;
    tab("products");
  });
}

function pickSiteImage(input,type){
  const file=input.files&&input.files[0];
  if(!file)return;
  compressImage(file,data=>{
    if(type==="logo")D.site.logo=data;
    if(type==="hero")D.site.hero=data;
    tab("site");
  });
}
...
async function adminLogin(){
  ...
  if(r.ok){
    sessionStorage.setItem("sweetHomeAdmin","1");
    showAdmin();
...
function adminLogout(){
  sessionStorage.removeItem("sweetHomeAdmin");
  closeAdmin();
}
...
function tab(t){
  ...
  if(t==="site"){
    $("adminContent").innerHTML=`<div class="admin">
      ...
      <label>اللوجو من المعرض</label>
      <input type="file" accept="image/*" onchange="pickSiteImage(this,'logo')">
      ${D.site.logo?`<img src="${e(D.site.logo)}" alt="اللوجو" style="width:120px;height:90px;object-fit:contain;background:#fff;border-radius:12px;margin:8px 0">`:""}

      <label>صورة البانر من المعرض</label>
      <input type="file" accept="image/*" onchange="pickSiteImage(this,'hero')">
      ${D.site.hero?`<img src="${e(D.site.hero)}" alt="البانر" style="width:100%;max-width:360px;height:140px;object-fit:cover;border-radius:12px;margin:8px 0">`:""}
  ...
  if(t==="products"){
    $("adminContent").innerHTML=D.products.map((p,i)=>`<div class="admin">
      ...
      <div class="row">
        ...
        <div>
          <label>صورة المنتج من المعرض</label>
          <input type="file" accept="image/*" onchange="pickImage(this,${i})">
          ${p.image?`<img src="${e(p.image)}" alt="" style="width:90px;height:70px;object-fit:cover;border-radius:12px;margin-top:8px">`:""}
        </div>
      </div>
    </div>`).join("")+`<button onclick="newProduct()">+ إضافة منتج</button>`;
  }
  ...
  if(t==="branches"){
    ...
    </div>`).join("")+`<button onclick="newBranch()">+ إضافة فرع</button>`;
  }
}
